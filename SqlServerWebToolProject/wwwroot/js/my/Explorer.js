...
$(function() {
	...
	$("#ulTree").tree({ checkbox: true, animate: true, 	onClick: TreeNode_Click, onContextMenu: BindTreeNodeContextMenu });
...
function NodeIsLeaf(node){
	...
	return (node.attributes.NodeFlag == g_node_sp || node.attributes.NodeFlag == g_node_tbl 
						|| node.attributes.NodeFlag == g_node_view || node.attributes.NodeFlag == g_node_func);
}

// 判断节点是不是一个目录节点。它应该是包含 “叶子节点” 的节点。但不是“数据库”节点
function NodeIsCategory(node){
	return (node.attributes.NodeFlag == "Tables" || node.attributes.NodeFlag == "Views" 
						|| node.attributes.NodeFlag == "Procedures" || node.attributes.NodeFlag == "Functions");
}

function btnRefresh_click(){
	if( $('#hidden_ConnectionId').val().length == 0 ){
		$.messager.alert(g_MsgBoxTitle, '没有指定连接参数。', 'error'); return false;
	}
...
function ShowResultInTab(url, dataType, param, title, nodeType, showInNewTab) {
	$(".waitMessageStyle").show();
	$.ajax({
		type: "GET",  dataType: dataType, cache: false, url: url, data: param,
		...
		success: function(result){
			var html = (nodeType == "Table")
				...
				: "<div class='h3Title'>(" + nodeType + ") " + title + "</div><pre class='brush: sql;' name='code'>" + result.replace(/</g, "&lt;") + "</pre>";
			if( showInNewTab )
...
function RefreshNodesForOneDB(){
	...
	$.ajax({
		...
		success: function(json){
			...
			for(var i=0;i<children.length;i++) if( NodeIsLeaf(children[i]) ) $("#ulTree").tree("remove", children[i].target);
			var children = $("#ulTree").tree("getChildren", root.target);
	...
	$('#divSearchDialog').show().dialog({
		...
		buttons:[
		{
			...
			handler: function(){ 
				...
				if( str.length == 0 ){ 
					$.messager.alert(g_MsgBoxTitle, '请输入要搜索的关键词。','warning', function(){$("#txtSearchWord").focus();}); return false; 
				}
				
				var flag = '';
				$("#tdSearchScope :checkbox:checked").each(function(){	flag += $(this).parent().text(); });
				if( flag.length == 0 ){
					$.messager.alert(g_MsgBoxTitle, '请选择要搜索的数据库对象类别范围。','warning'); return false;
				}
		...
		success: function (json) {
			...
			$.each(json, function(i, item){
				...
				html = html + 
					...
					"	<tr><td colspan='2'><pre class='brush: sql; first-line: 1; highlight: [" + item.LineNumber + "]' name='code'>" +
	...
	for(var i=0; i < nodes.length; i++){
		...
		if( root != null ){
			if( databaseName == null ){
				...
				$.messager.alert(g_MsgBoxTitle, '不能同时查看多个数据库的项目。','error');  return false;
			}
			
			if(      node.attributes.NodeFlag == g_node_sp )  { spNames   += ';' + node.text; count++; }
			else if( node.attributes.NodeFlag == g_node_tbl ) { tblNames  += ';' + node.text; count++; }
			else if( node.attributes.NodeFlag == g_node_view ){ viewNames += ';' + node.text; count++; }
			else if( node.attributes.NodeFlag == g_node_func ){ funcNames += ';' + node.text; count++; }
		}
	}
		...
		$.ajax({
			...
			success: function(json){
				...
				for(var i=0;i<json.length;i++)
					html += "<div class='Table'><div class='h3Title'>(Table) " + json[i].TableName + "</div>" + json[i].Html + "</div>";
		...
		$.ajax({
			...
			success: function(json){
				...
				for(var j=0; j < json.length; j++){
					var node = json[j];
					html +=  "<div class='subTitle'>--(" + node.TypeText + ") " + node.Name + "</div><pre class='brush: sql;' name='code'>" + node.SqlScript.replace(/</g, "&lt;") + "</pre>";
				}
				$('#tabs').tabs('add',{
					title: databaseName + " Code_" + (g_tabIndex++).toString(),
					content: html,
...
function DeleteSelectedItems(){
	...
	for(var i=0; i < nodes.length; i++){
		...
		if( root != null ){
			if( databaseName == null ){
				...
				$.messager.alert(g_MsgBoxTitle, '不能同时操作多个数据库的项目。','error');  return false;
			}
	...
	$.messager.confirm(g_MsgBoxTitle, '确定要删除所有已勾选的 ' + count + ' 个项目吗？？', function(dlgResult){ if( dlgResult ) {
		$(".waitMessageStyle").show();
		$.ajax({
			...
			success: function(text){
				for(var i=0; i < nodes.length; i++){
					...
					if( node.attributes.NodeFlag == g_node_sp || node.attributes.NodeFlag == g_node_tbl 
						|| node.attributes.NodeFlag == g_node_view || node.attributes.NodeFlag == g_node_func ){
				...
				for(var i=0; i < children.length; i++){
					...
					if( NodeIsCategory(node) ){
						...
						$("#ulTree").tree("update", {target: node.target, text: node.attributes.NodeFlag + "(" + ccc.length + ")"});
					}
				}				
