...
jQuery.fn.SetComboBox = function() {
    return this.each(function() {
        ...
        jComboBox.combobox({
            ...
            onSelect: function() {
                jComboBox.val($(this).combobox('getValue')); /*$(this).combobox("hidePanel");*/
                jComboBox.change();
            }
        });
    });
...
function SetSearchTextbox(textboxId, hiddenId, pickButtonClick) {
    ...
    j_text.removeClass("myTextbox").addClass("myTextboxReadonly").css("width", (width - 42)).css("float", "left")
        .css("border", "0px").css("height", height).attr("readonly", "readonly"); //	.attr("disabled", "disabled");
    j_div.append(j_text);

    $("<a></a>").attr("title", "选择").addClass("floatButton").addClass("searchButton").appendTo(j_div)
...
function SetChoiceDbControls() {
    ...
    SetSearchTextbox("txtDestConn",
        "hfDestConnId",
        function() { ShowSelectedConnAndFillDbComboBox("txtDestConn", "hfDestConnId", "cboDestDB"); });
    $("#tblChoiceConn select").attr("panelHeight", "300").SetComboBox()
        .combobox({ valueField: 'Name', textField: 'Name' });
    $("#txtSrcConn").change(function() {
        if ($("#txtSrcConn").val().length == 0) $("#cboSrcDB").combobox("loadData", []).combobox("clear");
    });
    $("#txtDestConn").change(function() {
...
function Check2ConnDB() {
    ...
    if ($("#hfDestConnId").val().length == 0) {
        $.messager.alert(g_MsgBoxTitle, '请选择数据目标的连接帐号。', 'warning');
        return false;
    }
    if ($("#cboDestDB").combobox("getValue").length == 0) {
        $.messager.alert(g_MsgBoxTitle, '请选择数据目标的数据库。', 'warning');
        return false;
    }
    return true;
}