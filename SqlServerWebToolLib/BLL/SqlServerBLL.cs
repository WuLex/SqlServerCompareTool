...
namespace SqlServerWebToolLib.BLL
{
    public sealed class SqlServerBLL : BaseBLL
    {
        ...
        protected override void EnsureMinVersion(DbConnection connection)
        {
            ...
            {
                throw new System.Exception("本程序只支持 SQL Server 2005 及以上版本。");
            }
        }
        ...
        protected override ItemCode GetProcedureItem(DbConnection connection, string name)
        {
            string query =
                string.Format(
                    "SELECT definition FROM sys.sql_modules JOIN sys.objects ON sys.sql_modules.object_id = sys.objects.object_id AND type in ({1}) and name = '{0}'",
                    name, s_ProcedureType);
