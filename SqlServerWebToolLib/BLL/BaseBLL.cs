	...
	public abstract class BaseBLL
	{
		...
		protected string GetDbConnectionString(ConnectionInfo info)
		{
			if( info.SSPI )
				...
				return string.Format("server={0};user id={1};password={2}", info.ServerIP, info.UserName, info.Password);
		}
		...
		protected string GetDbConnectionString(ConnectionInfo info, string dbName)
		{
			...
			if( info.SSPI )
				...
				return string.Format("server={0};user id={1};password={2};database={3}", info.ServerIP, info.UserName, info.Password, dbName);
		}
